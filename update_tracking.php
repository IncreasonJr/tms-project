<?php
// update_tracking.php - Admin Tracking Console
// Conforms to spec.pdf requirements (Login required)
        ...
        <div class="form-group" style="flex-grow: 1;">
            ...
            <select id="trip_id" name="trip_id" onchange="document.getElementById('trip-selector-form').submit();" class="form-control">
                ...
                <?php foreach ($all_trips as $t): ?>
                    <option value="<?php echo $t['id']; ?>" <?php echo $selected_trip_id == $t['id'] ? 'selected' : ''; ?>>
                        <?php echo $t['trip_code'] . ' (' . explode(',', $t['origin'])[0] . ' -> ' . explode(',', $t['destination'])[0] . ')'; ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
...
<?php if ($trip): ?>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <div>
                ...
                <h3 style="color: white; font-weight: 800; font-size: 1.5rem; margin-top: 0.25rem; display: flex; align-items: center; gap: 0.5rem;">
                    <span><?php echo $trip['trip_code']; ?></span>
                ...
                <p style="color: var(--text-secondary); font-size: 0.85rem; margin-top: 0.5rem;">
                    Location: <strong style="color: white;"><?php echo $latest_update ? $latest_update['location'] : $trip['origin']; ?></strong> | 
                    Driver: <strong style="color: white;"><?php echo get_driver_name($trip['driver_id']); ?></strong>
                </p>
            </div>
            <a href="track.php?code=<?php echo $trip['trip_code']; ?>&search=1" target="_blank" class="btn btn-secondary" style="font-size: 0.8rem; padding: 0.5rem 1rem;">
    ...
    <div class="dashboard-row" style="grid-template-columns: 1fr 1fr;">
        ...
        <div class="dashboard-panel">
            ...
            <form action="update_tracking.php" method="POST" style="display: flex; flex-direction: column; gap: 1.25rem;">
                        ...
                        <div class="timeline-step <?php echo $step_class; ?>">
                            ...
                            <div class="timeline-info" style="padding: 1rem;">
                                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.5rem;">
                                    <strong style="color: var(--status-color); font-size: 0.9rem;"><?php echo $update['status']; ?></strong>
                                    <span style="font-size: 0.7rem; color: var(--text-muted);">
                                        <?php echo date('H:i A, M d', strtotime($update['created_at'])); ?>
                                    </span>
                                </div>
                                <div style="font-size: 0.75rem; color: white; font-weight: 600; margin-top: 0.25rem;">
                                    ...
                                    <span><?php echo $update['location']; ?></span>
                                </div>
                                <p class="timeline-desc" style="font-size: 0.8rem; margin-top: 0.4rem;"><?php echo $update['description']; ?></p>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
